    ...
    notifyNewRequest: async (req, res, next) => {
        try {
            ...
            const { data: companyConfig } = await supabaseAdmin
                ...
                .eq('id', companyId)
                .single();
            const maintenanceRoles = companyConfig?.notification_settings?.mantenimiento || ['conserje', 'admin'];

            // 1. Notificación Push (FCM) a roles configurados
            try {
                console.log(`🔄 Enviando notificación Push a roles: ${maintenanceRoles.join(', ')}...`);
                for (const rol of maintenanceRoles) {
                    await NotificationService.enviarPorRol(rol, {
            ...
            // 2. Notificación WhatsApp a roles configurados
            try {
                console.log(`🔄 Buscando teléfonos de roles ${maintenanceRoles.join(', ')} para WhatsApp...`);
